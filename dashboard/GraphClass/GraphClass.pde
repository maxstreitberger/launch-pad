      ...
      void Bar(float[] a ,int from, int to) {
          ...
          if(from<0){                                      // If the From or To value is out of bounds 
           for (int x=0; x<a.length; x++){                 // of the array, adjust them 
               rect(int(xPos+x*Width/(a.length)),
                    yPos+Height-2,
                    Width/a.length-2,
          ...
          for (int x=from; x<to; x++){
            ...
            rect(int(xPos+(x-from)*Width/(to-from)),
                     yPos+Height-2,
                     Width/(to-from)-2,
                     -a[x]/(yMax-yMin)*Height);
...
//         if(!ErrorFlag |true ){    // sort out later!
       ...
       void setSettings(String xLabel, String yLabel, String title, int xDiv, int xMax, int xMin, int yMax, int yMin){
          this.xLabel=xLabel;
          this.yLabel=yLabel;
          this.Title= title;  
          this.xDiv=xDiv;  
